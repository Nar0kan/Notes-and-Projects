...
def main() -> None:
    ...
    for word in book_words_dictionary:
        if word not in words_list:
            different_words.append(word)
    
    print("Words that are in the book but not in the words list: ", different_words)

    #united_words = tuple(list(book_words_dictionary.keys()) + list(words_dictionary.keys()))

    #for word in united_words:
    #    if not word in book_words_dictionary or not word in words_dictionary:
    #        different_words.append(word)

    #print("First 60 different words:\n", different_words[:60], sep="\n")
